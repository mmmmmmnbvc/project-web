...
// ตรวจสอบว่ามีการส่งค่ามาจาก URL หรือไม่
if (isset($_GET['name_food'])) {
    // เตรียมคำสั่ง SQL เพื่อค้นหาข้อมูลโดยใช้ชื่ออาหาร
    $stmt = $pdo->prepare("SELECT name_food, price, calorie, typ_id, id FROM Menu_PCK WHERE name_food = ?");
    $stmt->bindParam(1, $_GET["name_food"]); // ใช้ค่าที่มาจาก URL
    $stmt->execute(); // เริ่มค้นหา
    $row = $stmt->fetch(); // ดึงผลลัพธ์ (1 แถว)
} else {
    echo "ไม่มีข้อมูลชื่ออาหาร";
    exit;
}
?>
<!doctype html>
...
function myFunction() {
  ...
  if (x.className === "topnav") {
    ...
    x.className = "topnav";
      ...
      <article>
      <form action="editstockss.php" method="post">
    ชื่ออาหาร: <input type="text" name="name_food" value="<?=$row["name_food"]?>" required><br>
    ราคา: <input type="text" name="price" value="<?=$row["price"]?>" required><br>
    แคลอรี่: <input type="text" name="calorie" value="<?=$row["calorie"]?>" required><br>
    หมวดหมู่: <input type="text" name="typ_id" value="<?=$row["typ_id"]?>" required><br>
    <!-- เพิ่ม hidden input เพื่อส่ง ID ไปด้วย -->
