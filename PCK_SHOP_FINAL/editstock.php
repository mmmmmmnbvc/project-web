...
function myFunction() {
  ...
  if (x.className === "topnav") {
    ...
    x.className = "topnav";
      ...
      <?php
$stmt = $pdo->prepare("SELECT name_food, price, calorie, typ_id, id FROM Menu_PCK ");
$stmt->execute();
while ($row = $stmt->fetch()) {
    echo "ชื่ออาหาร: " . htmlspecialchars($row["name_food"]) . "<br>";
    echo "ราคา: " . htmlspecialchars($row["price"]) . "<br>";
    echo "แคลอรี่: " . htmlspecialchars($row["calorie"]) . "<br>";
    echo "หมวดหมู่: " . htmlspecialchars($row["typ_id"]) . "<br>";
    echo "<a href='editstocks.php?name_food=" . htmlspecialchars($row["name_food"]) . "'>แก้ไข</a>  ";
    echo "<a href='deletestock.php?id=" . $row["id"] . "'>ลบ</a>"; // ส่ง id ผ่าน URL
    echo "<hr>\n";
}
?>
