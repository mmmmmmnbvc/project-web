      ...
      <article>
        <div class="menu-container" id="menu-container">
            <?php
                ...
                $stmt->execute();
                $count = 0; // นับจำนวนรายการที่แสดง
                while ($row = $stmt->fetch()) { 
                    $hiddenClass = $count >= 5 ? 'hidden' : ''; // ถ้าจำนวนรายการมากกว่า 5 จะซ่อน
                    ?>
                    <div class="menu-item <?= $hiddenClass ?>">
        ...
        <button id="view-all" style="display: none;">ดูทั้งหมด</button> <!-- ปุ่ม "ดูทั้งหมด" -->
    </article>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (menuItems.length > 5) {
                viewAllButton.style.display = 'block'; // แสดงปุ่ม "ดูทั้งหมด"
            }

            viewAllButton.onclick = function () {
                for (let i = 0; i < menuItems.length; i++) {
                    menuItems[i].classList.remove('hidden'); // แสดงรายการที่ซ่อนอยู่
                }
                viewAllButton.style.display = 'none'; // ซ่อนปุ่ม "ดูทั้งหมด"
            };
        });
    </script>
