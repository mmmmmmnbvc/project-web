...
<html lang="en">
  ...
  <head>
    ...
    <style>
        ...
        /* ให้ body ครอบคลุมความสูงทั้งหมดของหน้าจอ */
        html, body {
            height: 100%;
            display: flex;
        ...
        /* ให้ section (หรือเนื้อหาหลัก) ใช้พื้นที่ว่างที่เหลือ */
        section {
            flex: 1;
        }

        /* การจัดการ footer ให้พอดีกับด้านล่าง */
        footer {
  ...
  <a href="javascript:void(0);" class="icon" onclick="myFunction()">&#9776;</a>
      ...
      <section>
        <div class="profile">
            ...
            <input type="text" id="searchInput" placeholder="ค้นหาชื่อผู้ใช้" onkeyup="searchUser()">
        </div>
        <script>
        function myFunction() {
        ...
        function searchUser() {
            ...
            // หากไม่มีคำค้นหาหรือคำค้นหาว่างให้ลบผลลัพธ์
            if (searchQuery === "") {
                document.getElementById("result").innerHTML = "";
                return; // หยุดการทำงานของฟังก์ชันหากไม่พบคำค้นหา
            }

            // สร้าง XMLHttpRequest object
            var xhttp = new XMLHttpRequest();

            // กำหนดฟังก์ชันที่จะทำงานเมื่อการโหลดข้อมูลเสร็จสมบูรณ์
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    // ตรวจสอบว่าองค์ประกอบ #result มีอยู่ใน DOM ก่อน
                    var resultDiv = document.getElementById("result");
                    if (resultDiv) {
                        // แสดงผลลัพธ์ที่ได้จากเซิร์ฟเวอร์ใน div ที่มี id="result"
                        resultDiv.innerHTML = this.responseText;
                    } else {
                        console.error("ไม่พบ #result ในหน้าเว็บ");
                    }
                }
            };

            // ส่งคำขอ GET ไปที่ไฟล์ searchad.php โดยใช้ค่าคำค้นหา
            xhttp.open("GET", "searchad.php?query=" + searchQuery, true);
            xhttp.send();
        }
        function confirmDelete(username) {
            var confirmAction = confirm("คุณแน่ใจหรือไม่ว่าต้องการลบผู้ใช้: " + username + "?");
            if (!confirmAction) {
                return false; // ยกเลิกการลบถ้าผู้ใช้กดยกเลิก
            }
        }
        </script>
      </section>
