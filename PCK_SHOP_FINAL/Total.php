...
function myFunction() {
  ...
  if (x.className === "topnav") {
    ...
    <table border='1' style="margin: auto; width: 80%;">
        <tr>
            <th>เวลาการชำระเงินล่าสุด</th>
            <th>จำนวนอาหารที่ขายได้</th>
            <th>ยอดรวมของรายได้</th>
        </tr>
        <?php
        $stmt = $pdo->prepare("WITH PaidOrders AS (SELECT  order_date, user_id, total_price FROM Order_PCK WHERE payment_status = 'Paid')SELECT order_date,user_id,total_price FROM PaidOrders UNION ALL SELECT 'Total' AS order_date, NULL AS user_id,SUM(total_price) AS total_price FROM PaidOrders; ");
        $stmt->execute();
