...
<html lang="en">
  ...
  <head>
    ...
    <style>
        ...
        section {
            margin-bottom: 50px; /* ปรับค่า px ตามต้องการเพื่อเพิ่มระยะห่าง */
        }

        footer {
            margin-top: 215px; /* เพิ่มระยะห่างด้านบนของ footer */
        }
...
<div class="topnav" id="myTopnav">
  ...
  <a href="javascript:void(0);" class="icon" onclick="myFunction()">&#9776;</a>
      ...
      <section>
        <div class="h">
            <h1>Summary of totals each month</h1>
        </div>
        <script>
function myFunction() {
  ...
  if (x.className === "topnav") {
    ...
    <table border='1' style="margin: auto; width: 80%;">
        <tr>
            <th>เดือน</th>
            <th>จำนวนอาหารที่ขายได้</th>
            <th>ยอดรวมของรายได้</th>
        </tr>
        <?php
        $stmt = $pdo->prepare("SELECT DATE_FORMAT(O.order_date, '%Y-%m') AS month,COUNT(O.id) AS total_orders,SUM(O.total_price) AS total_revenue FROM Order_PCK O GROUP BY month ORDER BY month;");
        $stmt->execute();
