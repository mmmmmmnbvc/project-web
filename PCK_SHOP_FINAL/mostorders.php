...
<html lang="en">
  ...
  <head>
    ...
    <style>
        ...
        footer {
            margin-top: 147px; /* เพิ่มระยะห่างด้านบนของ footer */
        }
    </style>
  </head>
...
function myFunction() {
  ...
  if (x.className === "topnav") {
    ...
    <table border='1' style="margin: auto; width: 80%;">
        ...
        <?php
        $stmt = $pdo->prepare("SELECT  U.name_cus, U.surname_cus,COUNT(O.id) AS total_orders FROM  User_PCK U JOIN  Order_PCK O ON U.id = O.user_id GROUP BY  U.id ORDER BY total_orders DESC LIMIT 5;");
        $stmt->execute();
        while ($row = $stmt->fetch()) { 
        ?>
        <tr>
            <td><?php echo $row['name_cus'] ?></td>
            <td><?php echo $row['surname_cus'] ?></td>
