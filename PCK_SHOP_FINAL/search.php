...
<form class="example" action="search.php" method="GET" style="margin:auto;max-width:300px">
  ...
  <button type="submit"><i class="btn btn-primary btn-md">ค้นหา</i></button>
      ...
      <article>
      <?php
      $searchTerm = isset($_GET['search2']) ? $_GET['search2'] : '';
      $stmt = $pdo->prepare("SELECT Menu_PCK.id, Menu_PCK.name_food, Menu_PCK.price, Menu_PCK.calorie 
                             FROM Menu_PCK 
                             JOIN Category_Type_PCK ON Category_Type_PCK.typ_id = Menu_PCK.typ_id 
                             WHERE Category_Type_PCK.typ_id = 'T003'
                             AND Menu_PCK.name_food LIKE :searchTerm");
